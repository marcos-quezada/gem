...
static int valid_menu_tree(const gem_rpc_menu_bar_req_t *req)
{
    ...
    WORD first = 0, used = 1;
    if (req->objects[ROOT].ob_next != NIL)
        return 0;
    while (first < used) {
        WORD parent = queue[first++];
        const OBJECT *obj = &req->objects[parent];
        WORD child = obj->ob_head;
        if ((child == NIL) != (obj->ob_tail == NIL))
            return 0;
        if (child == NIL)
            continue;
        for (;;) {
            if (child < 0 || child >= req->object_count || seen[child])
                return 0;
            seen[child] = 1;
            queue[used++] = child;
            if (child == obj->ob_tail) {
                if (req->objects[child].ob_next != parent)
                    return 0;
                break;
            }
...
#define REQUEST(name, type)                                                    \
    case GEM_RPC_##name:                                                       \
        expected = sizeof(type);                                               \
        break
    switch (opcode) {
        case GEM_RPC_AES_EXT:
            return size == sizeof(gem_aes_packet_t) && payload &&
                   gem_aes_validate(payload);
        case GEM_RPC_AES_TREE:
            expected = sizeof(gem_tree_packet_t);
            break;
        case GEM_RPC_BITMAP_PUT:
        case GEM_RPC_BITMAP_GET:
            expected = sizeof(gem_bitmap_chunk_t);
            break;
        case GEM_RPC_BITMAP_COPY:
            expected = sizeof(gem_bitmap_call_t);
            break;
        case GEM_RPC_VDI_EXT:
            return size == sizeof(gem_vdi_packet_t) && payload &&
                   gem_vdi_validate(payload);
        ...
        case GEM_RPC_VQT_FONTINFO:
            expected = sizeof(gem_rpc_handle_req_t);
            break;
        case GEM_RPC_VSF_COLOR:
        case GEM_RPC_VST_COLOR:
        case GEM_RPC_VSL_COLOR:
            expected = sizeof(gem_rpc_color_req_t);
            break;
...
#undef REQUEST
    if (size != expected || (size && payload == NULL))
        return 0;
