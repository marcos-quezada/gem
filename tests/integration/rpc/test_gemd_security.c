...
static void send_request(int fd, uint16_t opcode, const void *data,
                         uint32_t size)
{
    ...
    if (size)
        assert(send(fd, data, size, MSG_NOSIGNAL) == size);
}
...
static int32_t receive_reply(int fd, void *out, size_t size)
{
    ...
    if (reply.size)
        assert(recv(fd, out ? out : discard, reply.size, MSG_WAITALL) ==
               reply.size);
    return reply.status;
}
...
static void stalls(int healthy)
{
    ...
    slow = connect_peer();
    (void)start(slow);
    header.opcode = GEM_RPC_GRAF_HANDLE;
    header.size = 0;
    for (int i = 0; i < 10000; ++i) {
        ssize_t written =
            send(slow, &header, sizeof(header), MSG_NOSIGNAL | MSG_DONTWAIT);
...
static gem_rpc_menu_bar_req_t menu(void)
{
    ...
    for (i = 0; i < 7; ++i) {
        ...
        req.objects[i].ob_width = 100;
        req.objects[i].ob_height = 20;
    }
    req.objects[0].ob_next = NIL;
    req.objects[0].ob_head = 1;
    req.objects[0].ob_tail = 4;
    req.objects[1].ob_next = 4;
    req.objects[1].ob_head = req.objects[1].ob_tail = 2;
    req.objects[1].ob_type = G_BOX;
    req.objects[2].ob_next = 1;
    req.objects[2].ob_head = req.objects[2].ob_tail = 3;
    req.objects[3].ob_next = 2;
    req.objects[3].ob_type = G_TITLE;
    req.objects[4].ob_next = 0;
    req.objects[4].ob_head = req.objects[4].ob_tail = 5;
    req.objects[5].ob_next = 4;
    req.objects[5].ob_head = req.objects[5].ob_tail = 6;
    req.objects[5].ob_type = G_BOX;
    req.objects[6].ob_next = 5;
    req.objects[6].ob_type = G_STRING;
    req.objects[6].ob_flags = LASTOB;
    req.strings[0].object = 3;
    strcpy(req.strings[0].text, "File");
    req.strings[1].object = 6;
    strcpy(req.strings[1].text, "Exit");
    return req;
}
...
static void bad_menus(int fd)
{
    ...
    bad = good;
    bad.objects[1].ob_head = 1;
    assert(call(fd, GEM_RPC_MENU_BAR, &bad, sizeof(bad)) == -1);
    bad = good;
    bad.objects[3].ob_next = 3;
    assert(call(fd, GEM_RPC_MENU_BAR, &bad, sizeof(bad)) == -1);
    bad = good;
    bad.objects[6].ob_flags = INDIRECT | LASTOB;
    assert(call(fd, GEM_RPC_MENU_BAR, &bad, sizeof(bad)) == -1);
    bad = good;
    bad.objects[6].ob_flags = 0;
    assert(call(fd, GEM_RPC_MENU_BAR, &bad, sizeof(bad)) == -1);
    bad = good;
    bad.strings[1].object = 5;
    assert(call(fd, GEM_RPC_MENU_BAR, &bad, sizeof(bad)) == -1);
    ...
    for (int object = 0; object < 7; ++object) {
        for (int field = 0; field < 3; ++field) {
            for (WORD value = -2; value <= 8; ++value) {
                ...
                bad = good;
                if (field == 0)
                    bad.objects[object].ob_next = value;
                if (field == 1)
                    bad.objects[object].ob_head = value;
                if (field == 2)
                    bad.objects[object].ob_tail = value;
                valid =
                    gem_rpc_valid_request(GEM_RPC_MENU_BAR, &bad, sizeof(bad));
    ...
    puts(
        "invalid menu indices, cycles, pointer forms and terminators rejected");
}
...
static void drawing(int a, int b, int c)
{
    ...
    assert(bit(244, 114));
    {
        ...
        assert(call(b, GEM_RPC_VSF_INTERIOR, &fill, sizeof(fill)) ==
               FIS_HOLLOW);
...
static void timers(int a, int b)
{
    ...
    timer.flags = MU_TIMER;
    timer.tlc = timer.thc = 65535;
...
static void input_routing(const int clients[3], const WORD windows[3])
{
    ...
    for (int owner = 0; owner < 3; ++owner) {
        ...
        for (int i = 1; i <= 3; ++i) {
            ...
            send_request(clients[client], GEM_RPC_EVNT_MULTI, &request,
                         sizeof(request));
            (void)receive_reply(clients[client], &reply, sizeof(reply));
            if (client == owner)
                assert((reply.event & MU_KEYBD) &&
                       (reply.kr & 255) == 'a' + owner);
            else
                assert(!(reply.event & MU_KEYBD));
        }
    }
    puts("keyboard follows focus between three apps; background apps cannot "
         "consume it");
}
...
static void locks(int healthy)
{
    ...
    assert(receive_reply(healthy, NULL, 0) > 0);
    close(owner);
    puts("unbalanced, disconnected and abandoned update locks cannot strand "
         "peers");
}
...
static void modal_waits(int healthy)
{
    ...
    close(owner);
    assert(call(healthy, GEM_RPC_GRAF_HANDLE, NULL, 0) > 0);
    puts("alert/selector waits service other apps and cancel on owner "
         "disconnect");
}
...
int main(void)
{
    ...
    setbuf(stdout, NULL);
    input_listen();
    assert(stat(gem_rpc_socket_path(), &mode) == 0 &&
           (mode.st_mode & 0777) == 0600);
    assert(call(b, GEM_RPC_GRAF_HANDLE, NULL, 0) == 0);
    a_id = start(a);
    b_id = start(b);
    c_id = start(c);
    ...
    stalls(a);
    a_window = window(a, 40);
    b_window = window(b, 220);
    c_window = window(c, 400);
    bad_menus(a);
